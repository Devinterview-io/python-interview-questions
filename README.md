...
Here's a Staff-level answer focusing on Python's key architectural features, structured for deep understanding and practical application.
...
2.  #### Bytecode Compilation
    ...
    *   **`compiler` Module**: Internally, CPython uses its `compiler` module (not exposed as a public API) to traverse the AST and generate corresponding bytecode instructions.
    *   **`.pyc` Files**: To optimize subsequent runs, Python often caches this bytecode in `.pyc` files (e.g., `__pycache__` directories). If the source file hasn't changed, the interpreter loads the `.pyc` file directly, skipping the parsing and AST generation steps.
...
3.  #### Python Virtual Machine (PVM) Execution
    ...
    *   **Global Interpreter Lock (GIL)**: A fundamental aspect of CPython's execution. The GIL is a mutex that protects access to Python objects, preventing multiple native threads from executing Python bytecode simultaneously. While it simplifies C extension development and memory management, it effectively means that CPU-bound Python code cannot achieve true multi-core parallelism using standard `threading`.
    *   **Memory Management**: CPython uses a combination of reference counting for immediate garbage collection and a generational garbage collector to break reference cycles.
...
1.  **Immutable Types** (`int`, `float`, `complex`, `bool`, `str`, `tuple`, `frozenset`, `bytes`, `NoneType`):
    ...
    *   `bytes` and `bytearray` handle sequences of raw 8-bit bytes, distinguishing binary data from text (`str`).
...
1.  **`list` vs. `tuple`**:
    ...
    *   **`tuple`**: Use for fixed collections, heterogeneous records (like database rows), or when immutability is required (e.g., for dictionary keys or set members). They are generally more memory-efficient and slightly faster for iteration or access than lists of the same size due to their fixed nature and lack of modification overhead. Their immutability also makes them suitable for representing constant data.
